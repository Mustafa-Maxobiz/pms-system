...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if (isset($channel->owner_id)) {
            $fetch       = $channel;
            $channelData = Chatify::getChannelWithAvatar($channel);
        } else {
            $fetch       = Chatify::getUserInOneChannel($request['channel_id']);
            $channelData = $fetch ? Chatify::getUserWithAvatar($fetch) : null;
        }

        if ($channelData) {
            if (! empty($channelData->profile_picture)) {
                $channel_avatar = asset('public/' . $channelData->profile_picture);
            } elseif (! empty($channelData->avatar)) {
                $channel_avatar = $channelData->avatar;
            } else {
                $channel_avatar = $channelData->avatar;
            }
        } else {
    ...
    public function send(Request $request)
    {
        ...
        $pusher = new Pusher(
            '761041d6cacccea14e85',
            '7557fab194e8f2a86d1e',
            '1931556',
            $options
        );
